	...
	<table bgcolor="#f8f9f9" cellspacing="0" border="0" cellpadding="0" width="100%" id="{{ \MailHelper::REPLY_SEPARATOR_NOTIFICATION }}" class="{{ \MailHelper::REPLY_SEPARATOR_NOTIFICATION }}" data-fs="{{ \MailHelper::REPLY_SEPARATOR_NOTIFICATION }}">
			...
			<td>
				...
				<table id="wrapper" align="center" width="95%" border="0" cellspacing="0" cellpadding="0" bgcolor="#ffffff" style="border: 1px solid #d4d9dd; max-width: 650px; border-bottom: 0; margin: 0 auto;">
	            	...
	            	@foreach ($threads as $thread)
												...
												<table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#f8f9fa">
													<tr>
														@if ($is_rtl)
															...
															<td valign="top">
																<div style="font-family:Arial, 'Helvetica Neue', Helvetica, Tahoma, sans-serif; color:#b5b9bd; font-size:12px; line-height:16px; margin:0; text-align: right; direction: rtl; unicode-bidi: plaintext;">
																	{!! $thread->getActionText('', true, false, $user, htmlspecialchars(view('emails/user/thread_by', ['thread' => $thread, 'user' => $user])->render())) !!}
																</div>
															</td>
														@else
															<td valign="top">
																<div style="font-family:Arial, 'Helvetica Neue', Helvetica, Tahoma, sans-serif; color:#b5b9bd; font-size:12px; line-height:16px; margin:0;">
																	{!! $thread->getActionText('', true, false, $user, htmlspecialchars(view('emails/user/thread_by', ['thread' => $thread, 'user' => $user])->render())) !!}
																</div>
															</td>
															<td valign="top">
	            		...
	            		@else
	            			{{-- Reply - body removed, View conversation link only --}}
							<tr>
								<td>
									            ...
									            <table width="100%" border="0" cellspacing="0" cellpadding="0">
									                ...
									                <tr>
									                    <td colspan="2">
									                        ...
									                        <p style="font-family:Arial, 'Helvetica Neue', Helvetica, Tahoma, sans-serif; color:#444; font-size:14px; line-height:20px; margin:0; @if ($is_rtl) text-align: right; direction: rtl; unicode-bidi: plaintext; @endif">
																<a href="{{ \Eventy::filter('email_notification.conv_url', $conversation->url(), $user) }}" style="color:#3f8abf; text-decoration:none;">{{ __('safeemailnotifications::messages.view_conversation') }}</a>
															</p>
									                    </td>
									                </tr>
									            </table>
		...
		<tr>
			<td height="0" style="font-size: 0px; line-height: 0px; color:#ffffff;">
				{{-- Addition to Message-ID header to detect replies --}}
